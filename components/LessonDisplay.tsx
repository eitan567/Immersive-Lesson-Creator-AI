...
const InfoCard: React.FC<{ icon: React.ReactNode; title: string; children: React.ReactNode }> = ({ icon, title, children }) => (
    <div className="div-cta rounded-xl">
        <div className="bg-white dark:bg-zinc-950 p-6 rounded-xl shadow-md h-full">
            <div className="flex items-center mb-4 pdf-flex-header">
                <div className="bg-pink-100 dark:bg-pink-900/50 text-pink-600 dark:text-pink-300 p-2 rounded-full ml-4 pdf-icon-wrapper">{icon}</div>
...
const LessonPartDisplay: React.FC<{ part: LessonPart; title: string; onImageClick: (url: string) => void }> = ({ part, title, onImageClick }) => (
    <div className="div-cta rounded-xl">
        <div className="bg-white dark:bg-zinc-950 p-6 rounded-xl shadow-md">
            <div className="flex items-center mb-4 pdf-flex-header">
                <div className="bg-pink-100 dark:bg-pink-900/50 text-pink-600 dark:text-pink-300 p-2 rounded-full ml-4 pdf-icon-wrapper"><BookOpenIcon className="w-6 h-6" /></div>
                <h3 className="text-2xl font-bold text-gray-800 dark:text-gray-100">{title}</h3>
            </div>
            <div className="space-y-4">
                <div className="flex items-center text-md text-gray-600 dark:text-gray-300 bg-gray-50 dark:bg-zinc-800/50 p-3 rounded-lg pdf-flex-header">
                ...
                {part.screens && part.screens.length > 0 && (
                    <div className="space-y-3 pt-3">
                         <h4 className="text-lg font-bold text-gray-800 dark:text-gray-200 flex items-center pdf-flex-header"><MonitorIcon className="w-5 h-5 ml-2 text-pink-600 dark:text-pink-400"/>מסכים</h4>
  ...
  return (
    <div className="pt-6 pb-6 px-6 md:px-4 lg:px-4">
      <header className="mb-8 pb-4 border-b-2 border-pink-200 dark:border-pink-800">
        ...
        <div className="flex items-center flex-wrap gap-x-4 gap-y-2 text-gray-600 dark:text-gray-300 pdf-flex-header">
      ...
      <div className="space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pdf-grid-cols-2">
            <InfoCard icon={<TargetIcon className="w-6 h-6" />} title="מטרות תוכן">
                <ul className="list-disc list-inside space-y-2 text-gray-700 dark:text-gray-300 pdf-list">
